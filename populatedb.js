...
function createGenreAuthors(cb) {
    async.parallel([
        ...
        function(callback){
          authorCreate('JK', 'Rowling', '1965-07-31', false, callback);
        },
        function(callback) {
...
function createBooks(cb) {
    async.parallel([
        ...
        function(callback) { //title, summary, isbn, author, genre, cb
          bookCreate('Harry Potter y el Prisionero de Azkaban', 'Un libro de un nino que va a una escuela magica donde aprende a ser un mago.', 'ISBN8498-38-6969', authors[5], [genres[0]], callback);
        }
        ],
        // optional callback
        cb);
...
function createBookInstances(cb) {
    async.parallel([
        function(callback) { //book, imprint, due_back, status, cb
        ...
        function(callback){
          bookInstanceCreate(books[7], 'Salamandra July, 2015', false, 'Available', callback)
        }
        ],
        // Optional callback
        cb);
